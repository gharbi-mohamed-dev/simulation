...
export function simulate({total,months,  previous, salary}: typeof SimulationSchema.Type) {
    const ra = 0.1
    ...
    const maxMontantFinacement = Math.round(
        ((Math.pow(v46+1, v45) - 1 ) * capNet) / (v46 * (2*v46+1) * Math.pow(v46 + 1, v45-1))
    )
    ...
    const monthly = (x43 * Math.pow(x44, x45) * v46) / (Math.pow(x44, v45) - 1)
